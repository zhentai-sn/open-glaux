...
describe("focusLayout（SDD feats/01 §9）", () => {
  ...
  it("缺省 → {railOpen:false, stageOpen:true}", async () => {
    const useSession = await freshStore();
    expect(useSession.getState().focusLayout).toEqual({ railOpen: false, rightOpen: true, rightView: "stage", railW: null, sideW: null });
  });
  ...
  it("字段类型不合法 → 逐字段回退默认", async () => {
    ...
    const useSession = await freshStore();
    expect(useSession.getState().focusLayout).toEqual({ railOpen: false, rightOpen: false, rightView: "stage", railW: null, sideW: null });
  });

  it("setFocusLayout 打补丁并持久化", async () => {
    const useSession = await freshStore();
    useSession.getState().setFocusLayout({ railOpen: true });
    expect(useSession.getState().focusLayout).toEqual({ railOpen: true, rightOpen: true, rightView: "stage", railW: null, sideW: null });
