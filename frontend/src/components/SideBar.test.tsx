...
describe("Explorer 三态（SDD 08 §11）", () => {
  it("无 active 数据源 → 空态卡，且不发任何数据请求", () => {
    ...
    for (const name of ["images", "naturalImages", "volumes", "slides", "taskRun"] as const) {
...
describe("模态切换器可见性来自数据源（SDD 08 D-1）", () => {
  it("只有一个 active 模态 → 不渲染切换器", () => {
    useSession.setState({ datasources: [ds("a", "carotid_imt")], images: [img("t1", "carotid_imt")] });
    ui();
    expect(screen.queryByRole("button", { name: "General images" })).toBeNull();
  });

  it("两个 active 模态 → 渲染两个候选；通用图像用中性标签且可选中", () => {
    useSession.setState({
      modality: "natural_image",
      datasources: [ds("a", "pathology"), ds("b", "natural_image")],
      naturalImages: [img("nat-1", "natural_image")],
      tasks: [
        {
    ...
    expect(screen.getByRole("button", { name: "Nuclei" })).toBeTruthy();
  });
...
describe("最近使用（SDD 08 §9.4）", () => {
  it("渲染最近项并可点击打开；不存在的对象被静默剔除", () => {
    useSession.setState({
      ...
      naturalImages: [img("nat-1", "natural_image")],
      recentItems: [
        { modality: "natural_image", id: "nat-1", label: "nat-1", at: "2026-08-30T00:00:00Z" },
        { modality: "natural_image", id: "gone", label: "gone", at: "2026-08-29T00:00:00Z" },
      ],
    });
    ui();
