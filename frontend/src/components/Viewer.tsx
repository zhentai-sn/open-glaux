...
import { axisFor, type ViewerEngine, type FrameAxis } from "../viewer/contract";
...
export function Viewer() {
  ...
  const onCoords = useCallback((coords: { x: number; y: number }) => setCoords(coords.x, coords.y), [setCoords]);
  ...
  const axisName = axisFor(object);
  const axis: FrameAxis = axisName
    ? { kind: axisName, index: focus.index[axisName] ?? 0, count: object.axes.find((entry) => entry.name === axisName)?.size ?? 1,
      onIndex: (index) => setIndex({ [axisName]: index }) }
    : { kind: "none" };
