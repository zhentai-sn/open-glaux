...
// 现在 polygon 一律是 PlanarFreehandROI，壁线形变归独立的 wall 工具（能力位限定 IMT）。
import { describe, expect, it } from "vitest";

import { activateTool, createToolGroup, destroyToolGroup } from "./csTools";
import { ImtWallHandleTool } from "./imtWallTool";
import { PlanarFreehandROITool, RectangleROITool } from "@cornerstonejs/tools";
...
describe("activateTool：工具 → CS3D 工具映射", () => {
  it("polygon 激活自由多边形，绝不激活壁线手柄", () => {
    ...
    expect(f.primaryTool()).toBe(PlanarFreehandROITool.toolName);
