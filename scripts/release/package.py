...
with zipfile.ZipFile(args.output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
    for name in ("compose.yaml", "start.sh", "stop.sh", "start.cmd", "stop.cmd", "start.command", "stop.command", "README.md", "README.zh-CN.md", "LICENSE"):
        archive.write(root / name, f"glaux/{name}")
    if args.images:
