...
| W4 | 查看器引擎收敛 | W3 | `ENGINES` 按 `ObjectKind` + 三注册面 + 画笔宿主合一 | 两引擎合并为 `FrameStackViewer`，`viewerEngines.smoke.test.tsx` 合并前后均绿，三注册面就位 |
...
- **W4 走完整合并**。W0 已写出 CS3D mock `RenderingEngine` smoke 测试（两引擎各覆盖渲染一帧、画一条多边形、提交一笔画笔），作为合并的回归网：用例指向合并后的组件，断言不变，每一步前后均须全绿（D-21）。
...
上表的 `size` 缺省 1024、上限 4096 与 `roi` 的 64 Mpx 阈值为本 SDD 冻结取值；W6 实测如需调整，按 §16 体例新增 `D-25` 起的决策行并同步修订本表、§13 与 §15.1 I 组。
...
| D-21 | 引擎合并以 CS3D mock `RenderingEngine` smoke 测试为前置回归网；登记的降级路径（只抽纯函数加共用 hooks 加手工回归清单、合并顺延独立立项）经 W0 实测不触发，W4 走完整合并 | 无回归网硬做合并；直接降级 | 两引擎合计 1044 行、此前零组件测试兜底，是全计划最高风险单步。W0 写出两引擎各三步的 smoke 用例，并以 7 处故意改坏的接线逐一被捕获验证其有效性 | 2026-09-23 |
...
以下两项已降为参数标定，不作为开放问题：无 `TaskView` 模态的能力位默认集（§9.4）与 `/objects/{id}/frame` 的 `size`／`roi` 上限（§5.2），二者均为本 SDD 冻结取值，W6 实测若需调整按 `D-25` 起的决策行修订。另有一项随任务结果面演进：`GET /volume/{id}/labelmap?task=&method=` 是否最终并入 `TaskOutput.ref` 的新形状而改名，超出本 SDD 范围，待任务结果面单独立项时裁定，本 SDD 不断言其删除时点。
