...
- 图像附件的缩略图：Composer 待发送区与会话流中的用户消息各一处，点击任一缩略图放大查看
  （overlay，点遮罩或 Esc 关闭；Esc 优先级高于快捷键速查面板）。只放大，不提供缩放平移——
  影像的导航与测量归查看器，不在对话里重复一套。
...
- [x] 点击 Composer 缩略图或消息里的图打开放大层；放大是查看而非移除，附件仍留在待发送列表。
- [x] 放大层点遮罩、点关闭按钮均可关闭；点图像本身不关闭；Esc 优先关放大层，其次速查面板，最后复位工具。
