...
- **R3 工具键需上下文**：无修饰单键（V/R/P/W/B）仅当**焦点落在查看器表面 `[data-viewer-surface]` 或页面主体 `body`（点击画布后的常态），且非可编辑元素**，并且当前引擎声明了该键时才生效；焦点在具体交互控件（侧栏按钮、输入框等）上时放行——避免全局误触，又无需给查看器强加焦点管理。
...
| 工具 · 框标注 | `R` | `setTool('bbox')` | 同上 |
| 工具 · 多边形 | `P` | `setTool('polygon')` | 同上 |
| 工具 · 壁线编辑 | `W` | `setTool('wall')` | 查看器聚焦 |
...
- **chat 版裁剪**：对话预览版只保留左会话栏（`Ctrl/Cmd+B`）与速查面板（`?`）两个快捷键（`SHORTCUT_ROWS` 按 edition 过滤），工具键与模式切换键不生效；裁剪口径由 [SDD 09](../09-chat-distribution/README.md) §14 规定，full 版保持本 SDD 原契约。
...
- [x] 焦点在查看器时按工具键（当前集 V/R/P/W/B）切换；焦点在文本输入框时按同键只输入不切换。（`globalKeys.test` 两用例）
