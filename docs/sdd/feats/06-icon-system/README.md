...
| 输入 | 说明 |
| --- | --- |
| 领域数据 | `tool.id`（cursor/editli/editma/roi/reset）、`capability.kind`（skill/model/dataset/…）、`FocusRightView`（stage/files/atlas）等——映射为图标名的键 |
...
| 站点 | 当前 glyph | 概念 |
| --- | --- | --- |
| `StatusBar` TOOL_GLYPH | `▸ ◠ ◡ ▭ ⟲` | 工具 cursor/editli/editma/roi/reset |
| `StagePanel` / `Editor` | `tl.glyph`（后端串） | 工具（改按 id 映射） |
| `FocusSidePanel` TABS | `▣ ▤ 📖` | 右侧栏 stage/files/atlas |
| `FocusTopBar` | `📁`、`⚙` | 图像选择器、连接配置 |
