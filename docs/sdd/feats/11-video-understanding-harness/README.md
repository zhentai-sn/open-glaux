...
3. 单次观察最长时长、Base64 字节预算、模型内部抽帧 `fps`、每轮调用次数和总媒体预算是多少？先按两家真实接口和 10 分钟样本标定。
...
7. Qwen 的 workspace 区域地址、MiMo 的请求字段和 pi-ai 现有仅文本／图像内容块的适配方式，经真实调用后确定。Qwen 默认 `reasoning_effort=xhigh`；低档在两个合成样本上答对且较快，但模型连接中思考力度的默认值与可配置范围须经更大样本和延迟预算确定。
