...
| --- | --- |
| 状态 | `ready` |
| 当前阶段 | 一期契约冻结，P1～P5 已实现；Qwen 的 Pi 工具循环已通过 9 秒蓝／绿／静音合成对照；待真实视频基本流程案例验收。标注集定量评测后置（D-15） |
...
`ready` 表示实现边界与验收方法已确定，不表示模型在真实视频上已经验收。业务尚未定性，一期验收只确认基本流程在真实视频上走通，不做定量精度评测。SDD 10 的 W4～W7 代码与自动化门禁已完成；其递延的浏览器走查和本 SDD 的端到端验收一起进行。§17 无开放问题。
...
- MiMo 及其他备用模型的一期接入或验收。新增模型须单独验证协议并走通同一基本流程案例，再修订本 SDD。
- 数小时视频的分层导航、完整视频播放器、实时流媒体、自动跟踪、剪辑、配音或自动摘要流水线。
- 语音转写及“画面加转写”回退；后续由用户配置转写服务时另定降级契约。
- Track/Event 的持久记录与复杂时序推断验证。
- 固定人工标注集与定量精度门槛（答案正确率、证据时间边界、区域 IoU、无答案拒答率）。业务定性并跑通基本流程后另立验收（D-15）。
...
| 目标 | 一期边界 |
| --- | --- |
| 输入 | 文件栏或对话框「添加文件」上传 MP4 或 WebM；单文件不超过 512 MiB，`0 < duration_ms ≤ 600000`；超限拒绝，不截断 |
...
| 对话框文件入口 | full edition 的 Composer；图片沿用 SDD 00 内联附件，视频复用 SDD 08 上传 | 视频上传成功后成为当前 `video` 对象，并在草稿中显示文件名；一条待发送问题最多关联一个视频，不把完整视频内联到 prompt |
...
一期验收以基本流程案例为准：维护者在 full edition 用至少一段真实有声视频走通「导入 → 提问 → Agent 多次观察 → 结构化证据 → 点击复核 → 刷新恢复」，并用一个与视频无关或前提错误的问题确认不给出有证据的虚假结论。固定标注集与定量阈值后置（D-15），不构成一期门禁。

- [ ] 上传 MP4、WebM（含有声与无声）均按容器和编码白名单受理；`>512 MiB`、`>600000 ms`、未知时长及坏编码逐文件拒绝，无半成品对象；大文件处理不随文件长度线性增长进程内存。
- [ ] 对话框「添加文件」可选图片或单个 MP4/WebM；视频立即导入并显示待发送引用，文字草稿不丢；发送时绑定该视频，失败或取消时不误发，chat edition 仍只有图片附件。
...
- [x] 每段片段时长、6 MiB 原始大小和严格小于 10 MB 的 Base64 限制均执行；每轮 12 次、600 秒、48 MiB 三项预算均执行，失败不静默截断或丢声。
- [x] 可变帧率、非零起始 PTS 和非关键帧起点样本的片段映射符合 §9 容差；引用超出观察、源时长、对象或区域边界时不能展示为有效证据。
- [ ] 基本流程案例：真实有声视频上 Qwen `reasoning_effort=low` 走通上述链路，回答与证据经维护者人工复核成立；无关或错误前提问题展示 `unanswered` 或明确否定前提；结构性越界引用零容忍。
...
| D-14 | 对话框「添加文件」复用现有上传与 Focus，图片仍内联，视频作为可寻址对象关联本轮问题 | 把完整视频作为消息附件发送 | 让用户在提问处选择视频，同时保留证据定位与原文件时间映射 | 2026-09-24 |
| D-15 | 一期验收只要求真实视频基本流程案例走通；固定标注集与定量精度门槛后置 | 一期即建 6 段视频／24 题标注集并按阈值准出 | 业务未定性，尚无跑通的基本流程案例，标注集题型与阈值无从确定 | 2026-09-25 |

## 17. 待确认问题

无。真实视频基本流程案例和完整浏览器走查是 §15 的验收门禁；若其结果推翻预算或模型选择，先修订本 SDD，再继续实现。标注集评测在业务定性后另行立项。
