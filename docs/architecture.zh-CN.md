...
| 查看器 | `components/Viewer.tsx` 按 `ObjectMeta.kind` 查 `ENGINES`：`image` → CornerstoneViewer，`volume` → VolumeViewer，`slide` → `viewer/PyramidViewer`（OpenSeadragon 原生标注叠加层）；缺键渲染「查看器引擎尚未接入」空态，不读 `TaskView.viewer`。2D / CT 合并仍在 W4 进行中 |
