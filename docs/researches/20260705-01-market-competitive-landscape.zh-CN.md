...
## 9. 生存期、护城河与定位再校准（对 Claude Code / Codex 等通用 agent 的防御）

> 这是决定 Glaux 能不能活 5–10 年的核心章节。前面 8 节回答"竞品是谁"；本节回答"当通用 agent 变得足够强，我们凭什么还在"。

### 9.1 通用 agent 会不会进来 / 泛化到能替代我们？

- **能力层：会，且赌它不会就是逆势下注。** 2028 年的通用 agent 一定能驱动 napari、写 Cellpose 流程、看图、吐分割——`napari-mcp` 今天已让 Claude/Cursor 直接控制 napari。**"我们把 NL→图像做得比通用 agent 好"这条路必输。**
- **产品层：不会。** Anthropic/OpenAI 的生意是水平平台 + 分销，不是去拥有生物实验室。他们的动作是放出通用能力 + 协议（MCP、computer-use），让垂直玩家在上面搭。
- **真正的威胁不是"OpenAI 做个生物影像产品"，而是"一个通用 agent + 开源工具（napari-mcp + Cellpose）免费复刻掉一个薄 wrapper"。** "NL 进、结构出"恰是那个最薄、最易被复刻的 wrapper——它窄不在领域，窄在它是个"功能"不是"平台"。
- **心智翻转**：把 Claude Code / Codex 从"竞争对手"重定义为"引擎 + 分销渠道"。对手是"薄 wrapper"这个**形态**，不是通用 agent 的推理。防御 = 别当 wrapper，去当它脚下的 substrate。

### 9.2 10 年护城河的唯一判据

能扛住 10 年模型持续变强的护城河只有一种：**模型越强、它越值钱**的资产。反之，模型变强让它趋近于零的，都是负债。用这把尺子量一切 roadmap 决策：

| 资产 | 模型变强时 | 判定 |
| --- | --- | --- |
| 薄 NL→脚本 wrapper、提示词链、硬编码流程 | 被商品化、趋近于零 | ❌ **负债** |
| **已验证的领域动作空间**（模态解码器 / 分割器注册表 / 带单位校准的测量原语 / 模态先验与护栏） | 更值钱——更聪明的 agent 有更好的"手和眼" | ✅ 护城河 |
| **验证 + 可复现 + provenance 底座** | 更值钱——agent 能做的越多，"结果可信"越关键 | ✅ 护城河 |
| **累积的已验证知识飞轮**（意图→流程→结果→人工校正→provenance，跨模态跨实验室） | 更值钱——独家评测集 / 微调数据 / "这类图该怎么处理"的先验 | ✅✅ **最强，会复利** |
| 分销 / 机构信任 / 工作流嵌入 / "研究非临床"监管姿态 | 中性偏正，水平 agent 拿不到 | ✅ 护城河 |

**那个 58% 可靠性天花板就是护城河入口。** 通用模型被训练成 helpful/plausible，**不是** rigorous/auditable——科学要"可验证、可复现、经得起同行评审"的输出。这是结构性、不随模型变强而消失的缺口。用**独家已验证数据**爬这道墙，爬的过程本身在挖别人下不去的沟。

### 9.3 四道护城河（按强度排序）

1. **已验证知识飞轮（数据 + 反馈，最强、会复利）**：每个在 Glaux 上跑的研究任务产出"意图→流程→结果→人工校正→provenance"。跨模态、跨实验室累积 → 独家评测集 + 领域微调/RAG 数据 + "这类图该怎么处理"的先验。通用 agent 拿不到，且直接攻击 58% 可靠性天花板。
2. **验证 / 可复现 / provenance 底座（信任）**：把验证和 provenance 做成**产品本身**而非附加项。模型越强越值钱。
3. **领域环境 / 动作空间（agent 动手所在的 substrate）**：模态解码器、分割器策略注册表、带单位/校准/尺度的测量原语、模态先验与护栏、QA 检查——这是通用 agent 需要的"手和眼"。**把 Claude Code/Codex 从威胁重定义为经 MCP 消费这套 substrate 的消费者。**
4. **分销 / 信任 / 工作流嵌入（GTM）**：机构关系、面向发表的可复现、真实实验室流程集成、开源社区信任、跨模态一致的"研究非临床"监管姿态。水平 agent 是水平的，不拥有生物实验室这层关系。

### 9.4 定位再校准：从"NL 进结构出"到双层模型

"NL 进、结构出"是**功能级**框架，且恰是最易被水平 agent 复刻的那层，必须抬高。但**单纯扩大 scope = 给水平 agent 更多可攻击面，除非锚在护城河上**。正解是**双层**：

> **上层（用户看到的愿景）**：一个 agent，给它研究目标，它交付**已验证的洞见**——会规划、动手、自我验证、迭代、跨实验记忆。这就是 **"Biomedical Image Insight × agent"**。（注意 Insight = 研究洞见，仍在决策上游，守住"研究非临床"边界。）
>
> **下层（让上层扛得住 10 年的地基）**：已验证领域动作空间 + 可复现/provenance 底座 + 复利的已验证知识飞轮 + 信任/分销。**这层正是水平 agent 没有、且模型越强越值钱的东西。**

一句话战略：**别在推理上跟通用 agent 卷。占住它推理所在的环境、和它学习所依赖的已验证数据。让通用 agent 当引擎，你当让它对科学可信的垂直层。**

### 9.5 执行铁律：愿景要宽，楔子要窄

"扩到 insight × agent"对的是**愿景/定位**；**首个落地楔子必须窄**——先在一个模态 / 一类工作流上把"agent 交付可验证洞见 + 飞轮转起来"跑通（如荧光显微的一类形态计量任务），证明飞轮与验证底座成立，再横向扩模态。创业公司死于过早铺开执行面，不死于愿景太大。**想平台，发楔子。**

### 9.6 对 Roadmap 的直接影响（在第 8 节基础上追加）

- **每个 roadmap 条目过一遍 9.2 的判据**：正在建的是"负债"还是"护城河"？薄编排层做够能用即可，别投入——把工程预算压到动作空间 + 验证底座 + 飞轮这三处。
- **飞轮从第一天就要转**：第一个 NL→分割闭环里，"人工校正"这一步不是 UI 细节，是**数据资产的入口**——校正必须被结构化捕获、可回流。这是 10 年复利的种子。
- **验证底座是一等目标不是后处理**：把"可诊断的失败 + 可重跑 + 审计轨迹"写进 README 第一条工程标准。
- **把 MCP 当朋友**：主动提供 Glaux-as-MCP-server，让通用 agent 通过它调用你的已验证动作空间——化威胁为分销。
