...
> 范围说明：本记录保留最初的双模型候选与 MiMo 文档核对。2026-09-24 用户将 MiMo 移出一期；当前实现与验收范围只以 SDD 11 为准，MiMo 未进行真实调用，也不是一期门禁。
...
这三段合成样本仍不能代表真实视频的一般问答准确率或引用精度。Qwen 在 Pi 中的接线、长短区间稳定性和人工标注集留待 SDD 11 实施验收；MiMo 真实调用随一期范围调整而递延。调用日志不得保存 API Key 或 Base64 视频正文。
