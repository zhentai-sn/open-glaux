...
### 由 SDD 10 承接（2026-09-23）

| ID | 状态 | 落点 |
| --- | --- | --- |
| D6 | 由 SDD 10 承接 | [SDD 10 视觉对象与数据源收敛](../sdd/feats/10-object-convergence/README.md)：数据源注册表收敛，W6 关闭 |
| D7 | 由 SDD 10 承接 | [SDD 10 视觉对象与数据源收敛](../sdd/feats/10-object-convergence/README.md)：模态收敛为一等对象类型，W6 关闭 |

### 尚未处理

D2（CI · 36，维护者本轮决定后置）、D9（覆盖率与 lint 规则集 · 20）、D10（部分完成：四个 `run-*.sh` /
`health.sh` 已随 `d14190d` 入库，2026-09-17 随 `b517c56` 移入 `scripts/dev/` 并补了目录说明；脚本里的仓库绝对路径和从 `~/.bashrc` 取 token 仍未处理）、D8、D11。
