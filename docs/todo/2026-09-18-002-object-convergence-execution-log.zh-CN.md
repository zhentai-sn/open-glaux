...
| WSI ROI 1 | 在真实浏览器打开 slide，瓦片与核质心叠加可见，控制台无 `unsafe-eval` 崩溃（F-18） | 部分通过：瓦片、核点由维护者确认；控制台待核 |
...
维护者提供的桌面浏览器截图显示 `slide_001` 瓦片与绿色 bbox ROI 可见；后端 `POST /annotations` 返回 201，查询到该手工 bbox 已保存。相同 ROI 的缓存检测返回 54 个核质心与计数/密度指标，`GET /wsi/slide_001/verify` 返回 200、F1=1.0。截图当时尚未显示核点或度量，可能摄于检测完成前；仍需维护者确认页面回流与控制台无 `unsafe-eval`。本段是辅助证据，不代替上表签字。

维护者随后确认已点击「复现验证」且无异常，WSI ROI 5 记通过；又确认当前 ROI 的核点叠加与数量/密度都显示。控制台异常与过小框提示仍待确认。
