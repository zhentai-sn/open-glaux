...
| WSI ROI 1 | 在真实浏览器打开 slide，瓦片与核质心叠加可见，控制台无 `unsafe-eval` 崩溃（F-18） | 通过：维护者截图确认瓦片、核点与 Console 无相关异常；F-18 关闭 |
| WSI ROI 2 | 缩放和平移后 bbox 框选仍对齐 level-0 座标，ROI 写入焦点 | 通过：维护者 2026-09-24 确认缩放后的框对齐 |
| WSI ROI 3 | polygon 绘制、顶点拖动、刷新回显位置一致 | 通过：维护者确认 WSI 新交互已开发好，刷新回显无问题；同时指出超声多边形手势不一致，转入跨模态修正 |
| WSI ROI 4 | 框选过小有提示；有效 ROI 触发核检测并更新度量 | 通过：维护者确认过小框提示、有效 ROI 的 54 个核点和数量/密度 |
| WSI ROI 5 | 验证按钮仍调用 `/wsi/{id}/verify` 并显示结果 | 通过：维护者 2026-09-24 实点「复现验证」，反馈无异常；接口返回 200、F1=1.0 |

其余手工项未签字，W4 暂不记完整准出；F-18 已关闭。SDD 10 §3 规定 W5 依赖 W2、W3，不依赖 W4，可并行推进。

### 2026-09-24 WSI 走查进展

维护者提供的桌面浏览器截图显示 `slide_001` 瓦片与绿色 bbox ROI 可见；后端 `POST /annotations` 返回 201，查询到该手工 bbox 已保存。相同 ROI 的缓存检测返回 54 个核质心与计数/密度指标，`GET /wsi/slide_001/verify` 返回 200、F1=1.0。截图当时尚未显示核点或度量，可能摄于检测完成前；后续维护者已确认回流结果。

维护者随后确认已点击「复现验证」且无异常，WSI ROI 5 记通过；又确认当前 ROI 的核点叠加与数量/密度都显示。Console 截图无 `unsafe-eval` 与查看器崩溃错误，WSI ROI 1 记通过，F-18 关闭。过小框提示仍待确认。

维护者继续走查确认 WSI ROI 2 的缩放后框对齐、ROI 4 的过小框提示均通过。ROI 3 报告多边形编辑刷新回显存疑，且希望逐点显示圆形顶点并更方便地闭合。修正设计见 [WSI 多边形交互修正设计](../plans/2026-09-24-wsi-polygon-interaction-design.md)；前端已按首点闭合、双击去重和拖动保存期间保持预览实施，待维护者在真实浏览器复验后签字。
