...
2. Focus 首屏 → 右侧栏应为**纯舞台 + 占位引导**；点「文件」标签 → 浏览器列在左、舞台在右，浏览器列显示 SDD 08 空态卡。
3. 拖 2 张 JPEG 进空态卡 → 图出现在树中并自动选中首张，**舞台同屏更新**，浏览器列未被关闭。
4. 连点另一张图 → 舞台跟着换，列表始终在。
5. 拖第三条分隔条到 240 与 480 两端 → 被夹住，舞台不小于 360px，无横向滚动。
...
| 项 | 说明 | 处理 |
| --- | --- | --- |
| 模态切换器在 240px 窄列的排版 | 加载示例后可能有 4–5 个候选，横向分段在窄列会挤。两份 SDD 都未规定窄列下的排版形态 | **不在本轮私自发明**。B 波走查若确认不可用，回 SDD 08 §5.4 补一条（如窄列改下拉），再实现 |
